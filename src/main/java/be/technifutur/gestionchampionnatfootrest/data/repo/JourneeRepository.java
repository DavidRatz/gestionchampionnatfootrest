...
public interface JourneeRepository extends JpaRepository<Journee,Long> {
    ...
    @Query(value = "select * from journee j join rencontre r on r.journee_id = j.id join club c on c.id = r.club_domicile_id join championnat ch on ch.id = j.championnat_id where j.numero = ?1 and j.championnat_id = ?2 order by r.date", nativeQuery = true)
